
<!DOCTYPE html>
<html lang="en">
<head>
  <title>PHP Index Page</title>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">

  <!--bootstrap5 library linked-->
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
  <link rel="stylesheet" href="css/list-of-products.css">
<body>
    
<!----------------featured categories -------------------->
  <div class="categories">
    <div class="small-container-fluid">
      <div class="row">
        <div class="col-sm-6 col-md-3">
          <img src="images/Book.jpg" alt="Book" />
        </div>
        <div class="col-sm-6 col-md-3">
          <img src="images/Book_4.jpg" alt="Book 4" />
        </div>
        <div class="col-sm-6 col-md-3">
          <img src="images/Book_2.jpg" alt="Book 2" />
        </div>
        <div class="col-sm-6 col-md-3">
          <img src="images/Book_2.jpg" alt="Book 2" />
        </div>
      </div>
    </div>
  </div>
<!----------------featured products -------------------->
  <div class="small-container-fluid">
    <h2 class="title">Featured Titles</h2>
      <div class="row">
        <div class="col-sm-6 col-md-3">
          <a href="book-detail.html">
            <img src="images/Book.jpg" alt="Book"
          /></a>
          <a href="book-detail.html"> <h4>The Godfather: 50th Anniversary Edition</h4></a>
          <p>$14.99</p>
        </div>
        <div class="col-sm-6 col-md-3">
          <a href="#">
            <img src="images/Book_2.jpg" alt="Book 2"
          /></a>
          <h4>The Great Gatsby</h4><br>
          <p>$4.99</p>
        </div>
        <div class="col-sm-6 col-md-3">
          <a href="#">
            <img src="images/Book_3.jpg" alt="Book 3"
          /></a>
          <h4>To Kill a Mockingbird</h4><br>
          <p>$7.99</p>
        </div>
        <div class="col-sm-6 col-md-3">
          <a href="#">
            <img src="images/Book_4.jpg" alt="Book 4"
          /></a>
          <h4>Alice in Wonderland</h4><br>
          <p>$5.99</p>
        </div>
      </div>

      <h2 class="title">Bestsellers</h2>
      <div class="row">
        <div class="col-sm-6 col-md-3">
          <a href="productdetail-page.html">
            <img src="images/Book.jpg" alt="Book"
          /></a>
          <h4>The Godfather: 50th Anniversary Edition</h4>
          <p>$14.99</p>
        </div>
        <div class="col-sm-6 col-md-3">
          <a href="productdetail-page.html">
            <img src="images/Book_2.jpg" alt="Book 2"
          /></a>
          <h4>The Great Gatsby</h4><br>
          <p>$4.99</p>
        </div>
        <div class="col-sm-6 col-md-3">
          <a href="productdetail-page.html">
            <img src="images/Book_3.jpg" alt="Book 3"
          /></a>
          <h4>To Kill a Mockingbird</h4><br>
          <p>$7.99</p>
        </div>
        <div class="col-sm-6 col-md-3">
          <a href="productdetail-page.html">
            <img src="images/Book_4.jpg" alt="Book 4"
          /></a>
          <h4>Alice in Wonderland</h4><br>
          <p>$5.99</p>
        </div>
      </div>
  </div>

</body>
</html>

